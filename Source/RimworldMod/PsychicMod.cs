...
namespace RimWorld
{
    public class PsychicMod : ModBase
    {
        ...
        public static void addBrainBurn(Pawn user, PsychicPowerDef power)
        {
            ...
            if (burn != null)
            {
                burn.Severity = burn.Severity + (power.brainBurnCost *
                                                 (1.2f - (0.025f *
                                                          user.GetStatValue(StatDefOf.PsychicEntropyRecoveryRate))));
            }
            else
            {
                burn = HediffMaker.MakeHediff(HediffDef.Named("PsychicBrainBurn"), user, brain);
                burn.Severity = power.brainBurnCost *
                                (1.2f - (0.025f * user.GetStatValue(StatDefOf.PsychicEntropyRecoveryRate)));
